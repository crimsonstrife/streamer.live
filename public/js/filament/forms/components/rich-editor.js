...
const $i = navigator.userAgent.match(/android\s([0-9]+.*Chrome)/i)
const Sn = $i && parseInt($i[1])
const xe = {
...
const fi = (i) => document.activeElement !== i && Tt(i, document.activeElement)
var Tt = function (i, t) {
  if (i && t) {
    for (; t;) {
      if (t === i) return !0
  ...
  return t
    ...
    : qt(i) ||
              (!qt(i.firstChild) &&
                  (function (e) {
                    return (
                      de().includes(j(e)) && !de().includes(j(e.firstChild))
                    )
                  })(i))
}
var qt = (i) => vo(i) && i?.data === 'block'
...
const Lt = (i) => Ir(i, wt)
const Or = (i) => ge(i) && i?.data === ''
var ge = (i) => i?.nodeType === Node.TEXT_NODE
const bi = {
  level2Enabled: !0,
  getLevel () {
...
const L = (i) => W[i]
var To = () => (Nn || (Nn = Object.keys(W)), Nn)
const ti = (i) => It[i]
var wo = () => (In || (In = Object.keys(It)), In)
...
const yi = new RegExp('[^\\S'.concat(bt, ']'))
const xi = (i) =>
  i
...
let { freeze: V, seal: Y, create: Vr } = Object
...
const Ne = G(Array.prototype.forEach)
const or = G(Array.prototype.pop)
const oe = G(Array.prototype.push)
const _e = G(String.prototype.toLowerCase)
const Fn = G(String.prototype.toString)
const sr = G(String.prototype.match)
const se = G(String.prototype.replace)
const _o = G(String.prototype.indexOf)
const jo = G(String.prototype.trim)
const X = G(Object.prototype.hasOwnProperty)
const _ = G(RegExp.prototype.test)
const ae =
    ((ar = TypeError),
    function () {
...
const zo = Y(/\{\{[\w\W]*|[\w\W]*\}\}/gm)
const qo = Y(/<%[\w\W]*|[\w\W]*%>/gm)
const Ho = Y(/\$\{[\w\W]*}/gm)
const Jo = Y(/^data-[\-\w.\u00B7-\uFFFF]+$/)
const Ko = Y(/^aria-[\-\w]+$/)
const zr = Y(
...
const Go = Y(/^(?:\w+script|data):/i)
const $o = Y(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g)
const qr = Y(/^html$/i)
const Yo = Y(/^[a-z][.\w]*(-[.\w]+)+$/i)
...
const Ve = (function i () {
  ...
  const M = u.prototype
  const pt = le(M, 'cloneNode')
  const Ct = le(M, 'remove')
  const Zt = le(M, 'nextSibling')
  const Qt = le(M, 'childNodes')
  const F = le(M, 'parentNode')
  ...
  const st = 'http://www.w3.org/1999/xhtml'
  let Ut = st
  let yn = !1
  let xn = null
  const co = b({}, [ke, Re, st], Fn)
  ...
  const Vi = function (a) {
    let g = null
    if ((at(J.beforeSanitizeElements, a, null), En(a))) {
      return et(a), !0
    }
  ...
  const Hi = function (a) {
    ...
    for (; v--;) {
      const A = g[v]
      const { name: f, namespaceURI: D, value: lt } = A
      const nt = I(f)
      let B = f === 'value' ? lt : jo(lt)
      if (
        ((h.attrName = nt),
        (h.attrValue = B),
        (h.keepAttr = !0),
...
const is = 'style href src width height language class'.split(' ')
const rs = 'javascript:'.split(' ')
const os = 'script iframe form noscript'.split(' ')
const Kt = class extends R {
  static setHTML (t, e) {
    const n = new this(e).sanitize()
    const r = n.getHTML ? n.getHTML() : n.outerHTML
    t.innerHTML = r
  }
...
const be = class extends gt {
  ...
  getClassName () {
    ...
    return (
      e && t.push(''.concat(ft.attachment, '--').concat(e)), t.join(' ')
    )
  }
...
const qe = class extends gt {
  ...
  createAttachmentNodes () {
    const t = this.attachment.isPreviewable() ? ze : be
...
const He = class extends gt {
  ...
  createNodes () {
    const t = []
    const e = fe.groupObjects(this.getPieces())
    const n = e.length - 1
    for (let o = 0; o < e.length; o++) {
      ...
      const c = this.findOrCreateCachedChildView(qe, s, {
    ...
    if (this.block.isEmpty()) t.push(p('br'))
      ...
      const n =
                (e = L(this.block.getLastAttribute())) === null || e === void 0
                  ? void 0
                  : e.text
      const r = this.findOrCreateCachedChildView(He, this.block.text, {
        textConfig: n
      })
...
const Gt = class extends gt {
  ...
  render () {
    if (
      ...
      const t = fe.groupObjects(this.document.getBlocks(), {
        asTree: !0
      })
      Array.from(t).forEach((e) => {
...
var cs = (i, t) => fr(i.innerHTML) === fr(t.innerHTML)
...
var xt = class extends mt {
  ...
  removeProhibitedAttributes () {
    const t = this.attributes.slice(xt.permittedAttributes)
...
const $t = class extends dt {
  ...
  splitObjectsAtRange (t) {
    let e
    let [n, r, o] = this.splitObjectAtPosition(ps(t))
    return (
...
const As = K.textForStringWithAttributes(
    `
`,
    { blockBreak: !0 }
)
...
var xs = (i) => i.copyWithoutAttribute('blockBreak')
var br = function (i) {
...
`
      ...
      let H
      const tt = u.text.getTextAtRange([0, c])
      const M = T.text.getTextAtRange([C, T.getLength()])
      const pt = tt.appendText(M)
      H =
                l !== d &&
                c === 0 &&
...
`
  ...
  getCommonAttributesAtPosition (t) {
    ...
    const d = Object.keys(It).filter((C) => It[C].inheritable)
    for (e in u) {
...
const Mt = class extends R {
  ...
  parse () {
    try {
      ...
      const t = je(this.containerElement, { usingFilter: Es })
...
`)
  ...
  appendBlockForElement (t) {
    ...
    const n = Tt(this.currentBlockElement, t)
...
`,
  ...
  getBlockHTMLAttributes (t) {
    const e = {}
    const n = Object.values(W).find((r) => r.tagName === j(t))
    return (
...
var Ss = (i) => i.replace(new RegExp('^'.concat(yi.source, '+')), '')
var ks = (i) => new RegExp('^'.concat(yi.source, '*$')).test(i)
var Gr = (i) => /\s$/.test(i)
const Rs = [
  'contenteditable',
  'data-trix-id',
...
const Ds = Object.freeze({ __proto__: null })
...
const rt = class extends R {
  ...
  insertLineBreak () {
    ...
    if (t.shouldPrependListItem()) {
      const e = new z([t.block.copyWithoutText()])
      return this.insertDocument(e)
    }
...
`)
  ...
  insertHTML (t) {
    const e = Mt.parse(t).getDocument()
    const n = this.getSelectedRange()
    ...
    return (
      Array.from(t).forEach((n) => {
        ...
        if (
          ...
          const o = yt.attachmentForFile(n)
          e.push(o)
        }
      }),
...
`
    ...
    const s = new z([n.removeLastAttribute().copyWithoutText()])
    return (
...
const Ze = class {
  ...
  getSignificantNodesForIndex (t) {
    const e = []
    const n = je(this.element, { usingFilter: Os })
    let r = !1
    for (; n.nextNode();) {
...
const { lang: Vn, css: kt, keyNames: Ms } = Ce
const zn = function (i) {
  return function () {
...
`
      ...
      : ((t = li(n)), (e = li(r)))
    const o = t.filter((l, c) => l !== e[c]).map(ue)
    const s = e.filter((l, c) => l !== t[c]).map(ue)
...
var js = (i, t) => Sr(i) === Sr(t)
const Ws = new RegExp(
  '('.concat('\uFFFC', '|').concat(ln, '|').concat(bt, '|\\s)+'),
  'g'
)
var Sr = (i) => i.replace(Ws, ' ').trim()
const Yt = class extends R {
  constructor (t) {
    super(...arguments),
    (this.element = t),
...
let qn
q(Yt, 'events', {})
const { browser: Us, keyNames: Qr } = Ce
let Vs = 0
const Z = class extends Yt {
  constructor () {
...
const Xs = ''.concat(ui, ', ').concat(hi)
const cn = '[data-trix-dialog]'
const Zs = ''.concat(cn, '[data-trix-active]')
const Qs = ''.concat(cn, ' [data-trix-method]')
...
const sn = class extends R {
  ...
  hideDialog () {
    const t = this.element.querySelector(Zs)
    let e
    if (t) {
      return (
        t.removeAttribute('data-trix-active'),
...
const P = new WeakMap()
const ye = class extends HTMLElement {
  constructor () {
    super(),
