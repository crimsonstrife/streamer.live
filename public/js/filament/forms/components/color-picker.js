const c = (e, t = 0, r = 1) => (e > r ? r : e < t ? t : e)
const n = (e, t = 0, r = Math.pow(10, t)) => Math.round(r * e) / r
const nt = { grad: 360 / 400, turn: 360, rad: 360 / (Math.PI * 2) }
const F = (e) => G(v(e))
var v = (e) => (
  e[0] === '#' && (e = e.substring(1)),
  e.length < 6
...
const at = (e, t = 'deg') => Number(e) * (nt[t] || 1)
const it = (e) => {
  const r =
...
const J = it
var lt = ({ h: e, s: t, l: r, a: o }) => (
  (t *= (r < 50 ? r : 100 - r) / 100),
  { h: e, s: t > 0 ? ((2 * t) / (r + t)) * 100 : 0, v: r + t, a: o }
)
const X = (e) => ct(A(e))
const Y = ({ h: e, s: t, v: r, a: o }) => {
  const s = ((200 - t) * r) / 100
  return {
    h: n(e),
    s: n(
...
const d = (e) => {
  const { h: t, s: r, l: o } = Y(e)
  return `hsl(${t}, ${r}%, ${o}%)`
}
const $ = (e) => {
  const { h: t, s: r, l: o, a: s } = Y(e)
  return `hsla(${t}, ${r}%, ${o}%, ${s})`
}
var A = ({ h: e, s: t, v: r, a: o }) => {
...
var ct = ({ r: e, g: t, b: r, a: o }) => {
  const s = o < 1 ? b(n(o * 255)) : ''
  return '#' + b(e) + b(t) + b(r) + s
}
var G = ({ r: e, g: t, b: r, a: o }) => {
  const s = Math.max(e, t, r)
  const a = s - Math.min(e, t, r)
  const i = a
    ? s === e
      ? (t - r) / a
      : s === t
...
const h = (e, t) => e.replace(/\s/g, '') === t.replace(/\s/g, '')
const K = (e, t) => (e.toLowerCase() === t.toLowerCase() ? !0 : L(v(e), v(t)))
const Q = {}
const H = (e) => {
  let t = Q[e]
  return (
...
let m = !1
const O = (e) => 'touches' in e
const pt = (e) => (m && !O(e) ? !1 : (m || (m = O(e)), !0))
const W = (e, t) => {
  const r = O(t) ? t.touches[0] : t
...
const u = class {
  constructor (t, r, o, s) {
    ...
    const i = t.querySelector(`[part=${r}]`);
  ...
  set dragging (t) {
    const r = t ? document.addEventListener : document.removeEventListener;
  ...
  style (t) {
    t.forEach((r, o) => {
      for (const s in r) this.nodes[o].style.setProperty(s, r[s])
    })
  }
}
...
const w = Symbol('same')
const R = Symbol('color')
const et = Symbol('hsva')
const _ = Symbol('update')
const ot = Symbol('parts')
const g = Symbol('css')
const x = Symbol('sliders')
const p = class extends HTMLElement {
  ...
  set color (t) {
    if (!this[w](t)) {
      const r = this.colorModel.toHsva(t);
      (this[_](r), (this[R] = t))
    }
  }
  ...
  constructor () {
    super()
    const t = H(`<style>${this[g].join('')}</style>`)
  ...
  attributeChangedCallback (t, r, o) {
    const s = this.colorModel.fromAttr(o)
...
const C = class extends p {
  get colorModel () {
    return mt
  }
}
const V = class extends C {}
customElements.define('rgb-string-color-picker', V)
const k = class extends u {
  constructor (t) {
    super(
      t,
      'alpha',
