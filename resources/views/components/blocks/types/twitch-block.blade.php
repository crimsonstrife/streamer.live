<div class="p-3 border rounded twitch-block">
    ...
    <input type="text" wire:model="blocks.{{ $index }}.content.channel" id="channel-{{ $index }}" class="mb-2 form-control" placeholder="Enter Twitch channel name">

    <label for="chat-toggle-{{ $index }}">Show Chat?</label>
    <input type="checkbox" wire:model="blocks.{{ $index }}.content.chat" id="chat-toggle-{{ $index }}" class="mb-2 form-check-input">
