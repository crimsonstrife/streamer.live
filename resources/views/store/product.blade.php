                                ...
                                <div class="carousel-item @if($index === 0) active @endif">
                                    <div class="position-relative">
                                        ...
                                        @if($image->model_name || $image->model_size_worn || $image->model_height_cm || $image->model_description)
                                            ...
                                            <div id="info-{{ $loop->index }}" class="model-info position-absolute bottom-0 start-0 w-100 p-3 bg-dark bg-opacity-75 text-white rounded-top d-none">
                                                @if($image->model_name)
                                                    <p class="mb-1"><strong>Model:</strong> {{ $image->model_name }}</p>
                                                @endif
                                                @if($image->model_size_worn)
                                                    <p class="mb-1"><strong>Size Worn:</strong> {{ $image->model_size_worn }}</p>
                                                @endif
                                                @if($image->model_height_cm)
                                                    <p class="mb-1"><strong>Height:</strong> {{ floor($image->model_height_cm / 30.48) }}'{{ round(fmod($image->model_height_cm / 2.54, 12)) }}" ({{ $image->model_height_cm }} cm)</p>
                                                @endif
