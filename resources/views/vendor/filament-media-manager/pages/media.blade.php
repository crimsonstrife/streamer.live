@php
    $currentFolder = \App\Models\Folder::find($this->folder_id);
    if(filament('filament-media-manager')->allowSubFolders){
        $folders = \App\Models\Folder::query()
            ->where('model_type', \App\Models\Folder::class)
            ...
            @else
                    ...
                    <div class="flex flex-col justify-start gap-4 border dark:border-gray-700 rounded-lg shadow-sm p-2 w-full h-full">
                        <div class="flex flex-col items-center justify-center  p-4 h-full">
                            ...
                            @elseif(str($item->mime_type)->contains('video'))
                                <video src="{{ $item->getMediaUrl() }}"></video>
                ...
                <div>
                    <div class="flex flex-col justify-start w-full h-full">
                        ...
                        @if(str($item->mime_type)->contains('image'))
                            <a href="{{ $item->getMediaUrl() }}" target="_blank" class="flex flex-col items-center justify-center  p-4 h-full border dark:border-gray-700 rounded-lg">
                                <img src="{{ $item->getMediaUrl() }}" />
                            </a>
                        ...
                        @elseif(str($item->mime_type)->contains('video'))
                            <a href="{{ $item->getMediaUrl() }}" target="_blank" class="flex flex-col items-center justify-center  p-4 h-full border dark:border-gray-700 rounded-lg">
                                <video class="w-full h-full" controls>
                                    <source src="{{ $item->getMediaUrl() }}" type="{{ $item->mime_type }}">
                                </video>
                            </a>
