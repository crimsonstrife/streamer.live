<div class="p-6 bg-white rounded-lg shadow">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" wire:click="addBlock">Add Block</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div wire:sortable="updateBlockOrder" class="mt-4 space-y-4">
        @foreach ($blocks as $index => $block)
            <div wire:sortable.item="{{ $block['id'] }}" wire:key="block-{{ $block['id'] }}" class="p-4 bg-gray-100 rounded shadow cursor-move">
                ...
                @elseif ($block['type'] === 'image')
                    <input type="text" wire:model="blocks.{{ $index }}.content.url" class="w-full p-2 border" placeholder="Image URL">
                @elseif ($block['type'] === 'embed')
                    <input type="text" wire:model="blocks.{{ $index }}.content.embed_url" class="w-full p-2 border" placeholder="Embed URL">
                @endif

                <button type="button" wire:click="removeBlock({{ $block['id'] }})" class="mt-2 text-red-500">Remove</button>
            </div>
        @endforeach
    </div>
